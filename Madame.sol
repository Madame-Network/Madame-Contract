...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minDistribution = 5 * (10 ** 18); // 5 ADA minimum auto send
    uint256 public minimumTokenBalanceForDividends = 3200000 * (10**9); // user must hold 3,200,000 token
...
contract MADAME is Ownable, IBEP20 {
    using SafeMath for uint256;
    ...
    string constant _name = "MADAME BUNNY";
    string constant _symbol = "MADAME";
    uint8 constant _decimals = 9;
    uint256 _totalSupply = 32000000000 * (10 ** _decimals);
    ...
    bool public trade = false;
    mapping(address => bool) private presale;
    modifier tradable(address from, address to) {
        require(trade || presale[from] || presale[to], "Not tradable!");
        _;
    }
    ...
    function setAllowPresale(address[] memory _users) external onlyOwner {
        for(uint8 i = 0; i < _users.length; i++) {
            presale[_users[i]] = true;
        }
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal tradable(sender, recipient) returns (bool) {
